...
reg_poids_juv_meres = lm(M_IND ~ M_MOTHERS, data = data_lezard) # alternative
 ...
 # valeurs des poids des juvéniles (variable réponse) estimées à partir de l'équation de régression pour chaque valeur observée de poids des mères (variable explicative) => valeurs prédites
  
 
 predict(reg_poids_juv_meres, newdata = data.frame(M_MOTHERS = seq(2, 5, 0.1)))
 # valeurs prédites de la variable réponse, en fonction d'un ensemble de valeurs données pour la variable explicative
...
#(pour l'instant non attribuées (NA), qui vont être
#remplies plus tard)
...
#Quel résultat parmi les 100 000 générés
...
b <- mean(data_lezard$M_IND)-a*mean(data_lezard$M_MOTHERS)
 # assez proche des estimations empiriques et identiques aux sorties de "lm()"
...
# on vérifie grâce à ce graphique l'homoscedasticité et la linéarité des résidus: les résidus doivent être distribués de manière homogène de part et d'autre de 0, selon un nuage de point homogène (ligne rouge horizontale en 0 sur le second graphe)
...
# on vérifie ici la normalité des résidus
 ...
 # on vérifie ici la présence ou non de points d'influence, la ligne rouge doit rester horizontale (sinon: présence de forts points d'influence), les forts points d'influence apparaissent au-delà d'une distance de Cook de 0.5 (cette limite apparait sur le graphique par une ligne pointillée lorsqu'elle est franchise)
 ...
 # l'ensemble des graphiques visant à vérifier la validité du modèle peuvent être retrouvés via la commande suivante:
...
# on réalise une régression mais en ajoutant un outlier évident
...
# on visualise très bien la présence de l'outlier sur les graphiques de vérification
...
# normalité des résidus approximativement respectée, pas de points d'influence majeurs):
...
# très faible adéquation aux données, seulement environ 1% de la variance (de la masse des juvéniles)
...
# variation de masse des individus (valeur absolue du coefficient directeur supérieure)
